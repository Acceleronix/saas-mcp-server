...
export class EUOneAPIUtils {
	...
	static async getDeviceDetails(
		...
		return EUOneAPIUtils.safeAPICallWithTokenRefresh(env, async (token) => {
			...
			const url = `${env.BASE_URL}/v2/device/detail/base?${queryParams.toString()}`;
			console.log("📝 Device details request URL:", url);
	...
	static async getDeviceEvents(
		...
		return EUOneAPIUtils.safeAPICallWithTokenRefresh(env, async (token) => {
			...
			console.log("📡 Device events response status:", response.status);
